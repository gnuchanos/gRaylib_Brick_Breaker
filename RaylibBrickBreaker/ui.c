...
// Text ========================================================================================================================================================
GText CreateGText(char ThisText[64], char *TFontPath, int TFontSize, Vector2 TPosition, Color TColor) {
	GText ThisGText = {
		.ThisText = "",
		.ThisTextFont     = LoadFont(TFontPath),
		.ThisTextFontSize = TFontSize,
		.ThisTextPosition = TPosition,
		.ThisTextColor    = TColor,
	};

	strcpy(ThisGText.ThisText, ThisText);
...
void UpdateGText(GText *ThisGText, int LeftPlayer, Font font) {
	sprintf(ThisGText->ThisText, "Player Score: %d", LeftPlayer);
	int TextSizeX = MeasureTextEx(font, ThisGText->ThisText, ThisGText->ThisTextFontSize, 0).x;
	ThisGText->ThisTextPosition.x = (float)GetScreenWidth()/2-(float)TextSizeX/2;
}

void DrawGText(GText *ThisGText) {

	DrawTextEx(ThisGText->ThisTextFont, ThisGText->ThisText, ThisGText->ThisTextPosition, ThisGText->ThisTextFontSize, 0, ThisGText->ThisTextColor);
}
...
Button CreateButton(char *Text, char *TextFont, int TextSize, float X, float Y, Color TextColor, Color NormalColor, Color HoverColor, Color PressColor) { 
	Button ThisButton;
		...
		ThisButton.ButtonPressWaitTime = 1.0f;
...
bool UpdateButton(Button *ThisButton, Vector2 MouseCursor) {
	...
	if (ThisButton->ButtonPressCheck) {
		if (ThisButton->ButtonPressWaitTime > 0) {
